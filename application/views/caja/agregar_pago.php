...
">
      <div class="container-fluid">
        ...
        <form id="frmExamen" action="<?php echo base_url('cash-management-save');?>">
          <div class="row mt-2">
<!--      
            <div class="col-sm">
          ...
          <div class="row mt-2">
            <div class="col-sm">
                ...
                <div class="form-group">
                  <label for="Procedimiento">Procedimientos => &nbsp;&nbsp;S./ <span style="font-size: 18px;" class="badge badge-warning" id="calculoCosto">0</span></label>
                  <select name="procedimiento[]" id="procedimiento" class="form-control procedimientos" multiple="multiple">
                  </select>
                </div>
				<!--
                <div class="form-group">
                  <label for="exampleInputPassword1">Laboratorios</label>
                  <select name="laboratorio[]" id="laboratorio" class="form-control searchExamen" multiple="multiple">
                  </select>
                </div>
				-->
              ...
              <table id="misCitas" class="table table-bordered table-hover">
                  ...
                  ?>
                      <tr> 
                        ...
                        <td align="center">
                          <form action="<?php echo base_url('cash-management-save-discount');?>" method="POST" id="frmDescuento<?php echo $clave;?>" onsubmit="return confirm('¿Realmente desea agregar el Descuento?');">
                            ...
                            <button type="submit" <?php echo $row->idCita ? "" : "disabled" ;?> title="Aplicar descuento">Aplicar</button>
                            <input type="hidden" name="tipo" value="<?php echo $row->concepto;?>">
                            <input type="hidden" name="codigoInterno" value="<?php echo $row->codigo_interno;?>">
						...
						<!--
                        <td align="center">
                          <form action="<?php echo base_url('cash-management-delete');?>" method="POST" id="frmEliminar<?php echo $clave;?>" onsubmit="return confirm('¿Realmente desea Eliminarlo?');">
                            <button type="submit" class="btn btn-danger" title="Eliminar registro" <?php echo $row->idCita ? "" : "disabled" ;?> ><i class="fas fa-minus-circle"></i></button>
                            <input type="hidden" name="tipo" value="<?php echo $row->concepto;?>">
                            <input type="hidden" name="codigoInterno" value="<?php echo $row->codigo_interno;?>">
                            <input type="hidden" name="idCita" value="<?php echo $row->idCita;?>">
                            <input type="hidden" name="usuario" value="<?php echo $row->idUsuario;?>">
                        ...
                        <td align="center"><a href="javascript:void(0)" id="imprime" onclick="print_link('<?php echo $row->codigo_interno; ?>', '<?php echo $row->concepto; ?>', '<?php echo $row->idUsuario; ?>', '<?php echo isset($row->idCita) ? $row->idCita : "0"; ?>')" title="Imprimir Examen" target="_blank"><i class="fas fa-print"></i> Imprimir</a></td>-->
                       
                      </tr>
                  <?php } ?>
                    <tr>
                      <td align="right" colspan="2"><strong>TOTAL(S/.)</strong></td>
...
<script src="<?php echo base_url('plugins/select2/js/select2.full.min.js');?>"></script>
  ...
  $('.searchExamen').select2({
    ...
    function print_link(codigo, tipo, user, cita) {
      var mywindow = window.open('<?php echo base_url('cash-management/print_add/') ?>' + codigo + '/'+ tipo + '/' + user + '/' + cita, 'Imprimir', 'toolbar=1,scrollbars=1,location=1,statusbar=1,menubar=1,resizable=1,width=800,height=550,left = 390,top = 50');
