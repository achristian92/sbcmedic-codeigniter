...
<div class="wrapper">
  ...
  <div class="content-wrapper" style="background: transparent;">
    ...
    <section class="content">
      ...
      <form id="quickForm" method="post" action="<?php echo base_url('cash-management-general');?>">
        <div class="row">
          ...
          <div class="col">
            <div class="form-group">
              <label for="fechaInicial">Tipo Consulta</label>
              <select class="form-control" name="tipo">
                <option value="">Todos</option>
                <option value="PRO">Procedimientos</option>
                <option value="LAB">Exámens de Laboratorio</option>
              </select>
            </div>
          </div>
                  ...
                  <tbody>
                    <?php
                      foreach ($resultados as $clave => $row){
                        $clave++;
                    ?>
                      <!-- <tr style="background-color: <?php echo $row->tipo == "LAB" ? "#dbeec8;" : "#c5edfe"; ?>"> -->
                      <tr>
                        ...
                        <td><?php echo $row->descuento ? $row->descuento : 0;?></td>
                        <td><?php echo $row->paciente;?></td>
                        <td>
                          <?php if( $row->tipo == "LAB") { ?>
                            <input type="checkbox" name="ckbPagoLab" disabled class="ckbPagoLab" data-bootstrap-switch data-off-color="danger" data-on-color="success" data-on-text="SI&nbsp;&nbsp;&nbsp;" data-off-text="NO" data="<?php echo $row->codigo;?>" idreg="<?php echo $row->id;?>" <?php  if($row->pagoStatus == 1) echo "checked='checked'"; else echo ""; ?>  <?php if($idUsuario == 8687 || $idUsuario == 6436 || $idUsuario == 3914 || $idUsuario == 48) echo ""; else echo "disabled"; ?> <?php if($row->pagoStatus == 1) echo "disabled"; ?> />
                          <?php } else{ ?>
                            <input type="checkbox" name="ckbPago" disabled data-bootstrap-switch data-off-color="danger" data-on-color="success" data-on-text="SI&nbsp;&nbsp;&nbsp;" data-off-text="NO" idreg="<?php echo $row->id;?>"  codeinterno="<?php echo $row->codigo_asignacion;?>" usuario="<?php echo $row->idUsuario;?>" <?php  if($row->pagoStatus == 1) echo "checked='checked'"; else echo ""; ?> <?php if(  $idUsuario == 8687 || $idUsuario == 6436) echo ""; else echo "disabled"; ?> <?php if($row->pagoStatus == 1) echo "disabled"; ?> />  
                          <?php } ?>
                        </td>
                        <td>
                          <?php if( $row->tipo == "LAB") { ?>
                          <?php } else{ ?>
                          <a href="javascript:void(0)" id="imprime" onclick="window_print('<?php echo $row->codigo_asignacion? $row->codigo_asignacion : 0;?>', '<?php echo $row->id;?>')" title="Citas">Citas</a>
                          <?php } ?>
                        </td>
                        <td>
                        <?php if( $row->tipo == "LAB") { ?>
                          <?php } else{ ?>
                          <a href="javascript:void(0)" id="imprime" onclick="window_print_caja('<?php echo $row->id;?>')" title="Imprimir Cita"><i class="fas fa-print"></i> Imprimir</a>
                          <?php } ?>
                        </td>
...
<script>
  ...
  $('.ckbPagoLab').on('switchChange.bootstrapSwitch', function(e, data) {
      $.ajax({
        type: 'post',
  ...
  $('input[name="ckbPago"]').on('switchChange.bootstrapSwitch', function(e, data){
      ...
      data: { idReg : $(this).attr('idreg'), status : data },
      success: function (respuesta) {
        if (!respuesta) alert('Error! No se guardo.');
        //location.reload();
      },
...
/*   $('input[name="descansoMedico"]').on('switchChange.bootstrapSwitch', function(e, data){
  ...
  function window_print(code, idCod) {
    var mywindow = window.open('<?php echo base_url('cash-management/window-print/') ?>' + code + '/' + idCod, 'Imprimir', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,width=950,height=450,left = 390,top = 50');
  }

  function window_print_caja(code) {
      var mywindow = window.open('<?php echo base_url('management-box/print/') ?>' + code, 'Imprimir', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,width=950,height=650,left = 390,top = 50');
  }
