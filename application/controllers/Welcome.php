...
/* 		$this->db->select("FORMAT(sum(precio), 2) as monto");
   ...
   {
//	   throw new \RuntimeException("entroo");
//	   log_message(1,"entrooo");
		...
		if($this->Helper->permiso_usuario("gestionar_citas"))
		{
			...
			if ($query) {
				foreach($query as $row)
				{
					...
					if(isset($row_resultadoDisponible['descripcion'])){
						$rowDescripcion = $row_resultadoDisponible['descripcion'];
					}
					...
					'motivoTipoCita'	=> htmlspecialchars($rowDescripcion, ENT_QUOTES),
...
public function fechas_disponibles()
{
  ...
  if($this->input->post("especialidades") || $this->uri->segment(2) > 0)
  {
    ...
    $this->db->group_by(array("av.date", "av.idDoctor", "av.turno", "av.area" , "av.codigo_interno "));
    //$this->db->order_by("av.date, av.turn, av.idDoctor asc");
    $this->db->order_by(" av.date  asc, left(min( av.start_time ), 5) asc");
		...
		if($this->Helper->permiso_usuario("administracion_caja"))
		{
			...
			if ($query) {
				foreach($query as $row)
				{
					...
					if(isset($row_resultadoTitulo['descripcion'])){
						$rowTitulo = $row_resultadoTitulo['descripcion'];
					}
					...
					'titulo'	=> htmlspecialchars($rowTitulo, ENT_QUOTES),
					'name'	=> htmlspecialchars($row->name, ENT_QUOTES)
					);
...
*/
	...
	public function patientSummaryManagementReadyNew()
	{
		...
		public function descargarPdftest()
    {
		$this->validarSesion();
		...
		$html = $this->load->view('test/muestra', "", TRUE);

		// Cargamos la librería
		$this->load->library('pdfgeneratornew');
		// generamos el PDF. Pasemos por encima de la configuración general y definamos otro tipo de papel-Letter
		$this->pdfgeneratornew->generate($html, "test", false, "A4", "portrait", "", "");
	}
